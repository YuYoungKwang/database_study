...
DELETE FROM 테이블명 : DML 데이터 삭제, 조건에따라서 원하는 데이터만 삭제(WHERE 조건), commit/rollback
TRUNCATE TABLE 테이블명 : DDL 데이터 삭제, 테이블 전체 데이터 삭제, 삭제실행시 바로 적용(auto commit)
